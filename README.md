...
//下载模板
$ vue init yuanjie007/element-admin cms(项目名字)
...
```
然后在```service/api```下面添加对应的请求接口，可以按功能来划分模块，这样子也比较好找，来看下代码:
```js
...
方法(methods):
```js
//检查传过来的api是否有权限，参数为String|Array 数组的话只要有一个有权限就返回true, api的前缀都是/api/admin/
